...
$\boldsymbol{\Omega}$ by pushing movable obstacles. For obstacle $\Omega_m$, a
pushing mode is denoted by
$\boldsymbol{\xi}_m \triangleq (\mathcal{C}_m,\mathbf{u}_m)$, where
$\mathcal{C}_m = (c_i)_{i=1}^{N}$ is an ordered tuple of contact points with
$c_i \in \partial\Omega_m$, and the ordering of $\mathcal{C}_m$ defines the
robot-to-contact assignment. The variable
$\mathbf{u}_m \in \mathbb{R}^{2N}$ parameterizes the corresponding per-contact
inputs, which collectively induce the net body wrench applied to $\Omega_m$.
For simplicity, all robots are assumed to participate in each pushing mode,
with each robot maintaining one simultaneous contact. The admissible set of
pushing modes is denoted by $\Xi_m$ and is determined by contact reachability,
collision-free pre-contact configurations, and frictional feasibility.
Furthermore, the system evolution under a pushing mode is captured by the
transition operator below:
...
$\Phi$ models the joint evolution of robots and obstacles under contact and is
evaluated through physics simulation.
