...
from a start configuration~$\mathbf{s}_\texttt{V}^{\texttt{S}}\in \mathcal{W}$ to a goal configuration
$\mathbf{s}_\texttt{V}^{\texttt{G}}\in \mathcal{W}$. Since movable obstacles may obstruct the way, a
...
\end{equation}
where~$\mathcal{P}^W_\texttt{V}$ is a continuous curve connecting the start and goal inside
the free space, and~$\texttt{clr}(\mathcal{P}^W_\texttt{V})$ denotes its minimum clearance
to all surrounding obstacles.

%==============================
\subsection{Collaborative Pushing Modes}\label{ss:interaction_mode}
To enlarge the blocked gaps, the robots may actively reconfigure
$\boldsymbol{\Omega}$ by pushing movable obstacles.
For obstacle~$\Omega_m\in \boldsymbol{\Omega}$,
a pushing mode is denoted by
$\boldsymbol{\xi}_m \triangleq (\mathcal{C}_m,\mathbf{u}_m)$, where
$\mathcal{C}_m \triangleq \{c_1,\cdots,c_N\}$ is an ordered tuple of contact points
on the boundary of~$\Omega_m$ with $c_i \in \partial\Omega_m$, and the ordering of~$\mathcal{C}_m$ defines the
robot-to-contact assignment. The variable
$\mathbf{u}_m \in \mathbb{R}^{2N}$ are the per-contact
net body wrench applied to~$\Omega_m$.
All robots are assumed to participate in each pushing mode,
each maintaining one simultaneous contact. The admissible set of
pushing modes is denoted by~$\Xi_m$ and is determined by contact reachability,
pre-contact configurations, and pushing feasibility.
Furthermore, the system evolution under a pushing mode is captured by the transition below:
\begin{equation}\label{eq:transition}
  \mathbf{s}(t^+)\triangleq\Phi\big(\mathbf{s}(t),\,m,\,\boldsymbol{\xi}_m\big),
\end{equation}
where~$\mathbf{s}(t)$ stacks all robot and obstacle states;
and the operator~$\Phi$ encapsulates the overall evolution,
which can be evaluated through physics simulation.
