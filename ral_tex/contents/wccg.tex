...
where~$\oplus$ denotes Minkowski addition and~$\mathbb{B}_{W/2}$ is a disk of radius~$W/2$.
Then, the criterion for the  existence of a~$W$--clear path
$\mathcal{P}^W_\texttt{V}$ by~\eqref{eq:transition} is:
...
with~$\mathbb{B}_{W/2}(\cdot)$ being the centered disk of radius~$W/2$.
...
inward into~$\mathcal{F}_W$, and attaching short connectors inside the
...
\begin{remark}\label{remark:wccg}
  ...
  (I) It avoids discretization of~$\mathcal{W}$ and is therefore \emph{free from}
