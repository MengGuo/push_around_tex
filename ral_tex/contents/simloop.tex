...
physics-validated strategies through parallel candidate evaluation. It is a feasibility-oriented heuristic search, where Eq.~\eqref{eq:problem} defines the realized clearance cost of a partial schedule, the accumulated-cost term in the node priority of Eq.~\eqref{eq:priority}. 
\change{The search samples a finite candidate set, so neither global optimality nor
