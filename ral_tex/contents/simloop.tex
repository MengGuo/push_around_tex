...
validates multi-robot pushing strategies through short-horizon simulation, as shown in Fig.~\ref{fig:PIHS} and summarized in
Alg.~\ref{alg:pihs}. Unlike purely geometric NAMO planners, PIHS expands only physics-validated strategies in parallel. It is a feasibility-oriented heuristic search, where~\eqref{eq:problem} defines the realized clearance cost of a partial schedule, the accumulated-cost term in the node priority of~\eqref{eq:priority}. 
