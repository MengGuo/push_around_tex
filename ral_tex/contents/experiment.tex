...
\vspace{0.5mm}
{\scriptsize \change{$^\dagger$WCCG errors on CC--FR-consistent cases. All runtimes are
...
definition, and movable obstacles have masses sampled from $[5,15]$\,kg. A trial
...
Two representative simulation runs are shown in Fig.~\ref{fig:main_result}. In
Scenario~1 the start and goal lie in different $W$-clearance faces, so the
planner clears the blocked passage with $6$ pushes in $7.4$\,s and execution
finishes in $42.3$\,s. Scenario~2 requires a $9$-push plan in $11.2$\,s and
$61.3$\,s of execution. In both cases the executed pushes progressively enlarge
the reachable $W$-clearance region until a complete $W$--clear path is found, as
shown by the expanding blue start face in the WCCG overlays.
...
reference (CC) follows the classical configuration-space construction, inflating
obstacle and workspace boundaries by~$W/2$ and querying connectivity with a fine
continuous collision check at $0.01$\,m oracle resolution instead of a fixed
grid~\cite{lozano1983spatial}; the rasterized reference (FR) discretizes the same
inflated workspace at $0.02$\,m for an occupancy-grid query. CC and FR disagree
...
($62.5\%$); SL-Push (sim) reaches $75.0\%$ at $18.2$\,s planning; and Rec-NAMO
usually fails to build a connected $W$--clear corridor in dense clutter, as
illustrated in Fig.~\ref{fig:baseline}.
...
whereas mode-cache only lowers success to $92.5\%$; quick-pass affects
efficiency, with simulations rising from $121.5$ to $208.2$ and planning from
$10.3$ to $23.6$\,s.
...
PushAround maintains a $100\%$ success rate for all $M\in[15,45]$, while its
mean planning time grows from $2.5$ to $53.4$\,s. At $M{=}45$, the three
external comparison methods solve none of the trials, whereas the no-recursion
ablation solves $7/10$.
\textbf{(II) Robot-team size:} at $M{=}30$, PushAround solves all ten trials at
$N{=}2$, $3$, and $4$ after the shared mode-generation refinements. The
refinements reduce robot-order sensitivity and allow a temporary per-task
robot release when a full-team assignment is unavailable; full-team modes are
preferred through a ranking penalty. At $N{=}4$, PushAround uses $29.2$\,s of
...
succeeds in $156$ of $160$ perturbed trials ($97.5\%$), with one low-friction
planning failure and three budget timeouts. Under the same $\pm40\%$
perturbations, SL-Push (sim) succeeds in $13/40$ trials and DFS-WCCG in
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.9\columnwidth]{figures/density_comparison.pdf}
  \vspace{-0.15in}
  \caption{\change{
...
Motion capture provides the current robot and object poses, which are compared
with the successor state predicted by $\mathsf{EvalSim}$: a push triggers
replanning only when the measured object position error exceeds $0.3$\,m or its
orientation error exceeds $\pi/4$, a deliberately loose threshold that tolerates
