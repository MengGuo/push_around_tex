...
in $4$ of the $300$ cases, which bounds the ambiguity of the ground truth itself;
on the $296$ scenes where they agree, WCCG matches CC with zero false positives
and zero false negatives. A query costs $1.07$\,ms against $1.36$\,ms
(CC) and $243$\,ms (FR), while additionally supplying the gap graph needed by
PIHS.
}
...
(Table~\ref{tab:generalization} and Fig.~\ref{fig:density_scaling}).
...
$N{=}2$, $3$ and $4$ after two mode-generation refinements: bounded backtracking
contact assignment removes the robot-order sensitivity of greedy replacement,
and a temporarily released robot is re-activated once a full-team assignment
exists, with a penalty favoring full-team modes. The fallback belongs to the
shared mode generator, so the baselines benefit from it as well, yet at $N{=}4$
PushAround stays the most economical with $29.2$\,s planning, $110.1$\,s
execution and $10$ pushes versus $101.9$\,s, $175.9$\,s and $14$ for DFS-WCCG.
\textbf{(III) Physics mismatch:} the planner keeps the nominal model and only the
execution simulator is perturbed, one parameter at a time: PushAround succeeds in
$156$ of $160$ perturbed trials ($97.5\%$), the four failures being one
low-friction planning failure and three budget timeouts. Under the same $\pm40\%$
perturbations SL-Push (sim) averages $32.5\%$ and DFS-WCCG $57.5\%$, at an order
of magnitude higher planning time.
