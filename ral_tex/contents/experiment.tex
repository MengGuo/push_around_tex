...
\change{\begin{tabular}{cccccc}
\hline
\multicolumn{3}{c}{} & \multicolumn{3}{c}{Time (ms)}\\
\cmidrule(lr){4-6}
...
\vspace{0.5mm}
{\scriptsize \change{$^\dagger$WCCG errors with respect to the continuous configuration-space
reference (CC). All runtimes are measured in one harness over the same $300$
scenes, with WCCG served by its compiled backend;}}
\vspace{-5mm}
\end{table}
...
definition, and movable obstacles have masses in $[5,15]$\,kg. A trial
...
($14$ obstacles) provides an additional cluttered case. \change{All methods share the
robot footprints, obstacle configurations, collision and reachability checks,
and execution simulator, with the same per-trial timeout of $800$\,s and
simulator cap of $20{,}000$ control steps; unsuccessful trials are assigned
the $800$\,s cap in the planning-time statistics.}

%==============================
\subsubsection{Results}

\change{PIHS expands a node by ranking the visible frontier gaps by their
estimated downstream cost. For each gap, the predicted gap sequence is obtained
and mode optimization generates candidate strategies covering direct pushes of
adjacent obstacles and recursive pushes of out-layer blockers. Each candidate
passes a geometric quick-pass and is then evaluated by a short-horizon
$\mathsf{EvalSim}$ rollout returning the successor state, time cost and
manipulation effort; validated candidates are appended to the partial schedule
and inserted as child nodes, and a batch is evaluated in parallel. The search
terminates once the start and goal share a $W$-clearance face.} Two
representative runs are shown in Fig.~\ref{fig:main_result}. \change{Scenario~1
returns a six-task schedule planned in $7.4$\,s and executed in $42.3$\,s, and
Scenario~2 a nine-task schedule planned in $11.2$\,s and executed in
$61.3$\,s. During execution the state is updated after each push and the WCCG is
rebuilt, so the reachable face expands until it contains the goal. The cost of
these runs is dominated by physics-based validation of candidates, whereas gap
ranking, quick-pass filtering, cached modes and parallel rollouts keep the
number of simulations small.}
...
WCCG is compared against two references on $300$ random scenes covering convex,
non-convex and mixed obstacles (Table~\ref{tab:wccg-validation}). The continuous
reference (CC) inflates obstacle and workspace boundaries by~$W/2$ and queries
connectivity with a fine continuous collision check at $0.01$\,m oracle
resolution instead of a fixed grid~\cite{lozano1983spatial}; the rasterized
reference (FR) discretizes the same workspace at $0.01$\,m on an occupancy grid. CC and FR disagree
in $4$ of the $300$ cases, indicating sensitivity of the raster reference near
narrow or tangential contacts. WCCG matches CC on all $300$ scenes with zero
false positives and zero false negatives. A query costs $1.07$\,ms against
$1.36$\,ms (CC) and $243$\,ms (FR), while additionally supplying the gap graph
needed by PIHS.
}
...
Four baselines run on Scenario~1 over $40$ randomized trials under the same
$W$-clearance criterion, object distribution and simulator:
...
rate ($97.5\%$) at the lowest combined planning-and-execution time ($38.9$\,s).
DFS-WCCG suffers from exponential branching, the offline SL-Push variant
produces displacements that fail in execution, and Rec-NAMO usually fails to
build a connected $W$--clear corridor in dense clutter, as illustrated in
...
\change{\begin{tabular}{l c c c c}
\hline
...
\begin{flushleft}
\footnotesize
\change{Each perturbed row scales one parameter; the others stay nominal.}
\end{flushleft}
\vspace{-6mm}
\end{table}
%==============================
...
whereas mode-cache only lowers success to $92.5\%$ and quick-pass affects
efficiency (simulations rise from $121.5$ to $208.2$).
...
execution-side physics parameters (Table~\ref{tab:generalization} and
Fig.~\ref{fig:density_scaling}).
\textbf{(I) Obstacle density:} with two robots and $10$ trials per density,
PushAround keeps a $100\%$ success rate for all $M\in[15,45]$ at $2.5$--$53.4$\,s of
planning, whereas at $M{=}45$ the external methods solve none of the trials and the
no-recursion ablation solves $7/10$.
\textbf{(II) Robot-team size:} at $M{=}30$, PushAround solves all ten trials at
$N{=}2$, $3$, and $4$ after the shared mode-generation refinements, which reduce
robot-order sensitivity and allow a temporary per-task robot release when no
full-team assignment exists, with a ranking penalty preferring full-team modes. At $N{=}4$, PushAround uses $29.2$\,s of
planning and $110.1$\,s of execution, compared with $101.9$\,s and $175.9$\,s
for DFS-WCCG.
...
succeeds in $156$ of $160$ perturbed trials ($97.5\%$); under the same $\pm40\%$
perturbations SL-Push (sim) succeeds in $13/40$ and DFS-WCCG in
$23/40$ trials; their mean planning times are $31.5$\,s and $229.1$\,s,
respectively, compared with $18.4$\,s for PushAround.
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/obs_rob_scale.png}
  \vspace{-0.09in}
  \caption{\change{
  Scalability of the clearing process. \textbf{Left:} obstacle-density scaling
  ($M{=}45$ objects, two robots): snapshots during the clearing (\textbf{top})
  and WCCG overlays with the start face in blue (\textbf{bottom}).
  \textbf{Right:} robot-team scaling ($M{=}30$ objects, four robots): initial
  and final states (\textbf{top}) and zoomed views of one push before contact
  and during execution (\textbf{bottom}).}}
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/density_comparison.pdf}
  \vspace{-0.15in}
  \caption{\change{
  ...
  denote one standard deviation across seeds.}}
...
\begin{figure*}[t!]
  \centering
  \includegraphics[width=0.84\linewidth]{figures/hardware.png}
  \vspace{-2mm}
  \caption{\change{Real-world pushing experiments with execution--planning alignment (over~$20$ runs and $2$ scenarios).
...
obstacles; robots and movables carry motion-capture markers whose poses are
streamed to \texttt{PyBullet} in real time.
\change{The two Mecanum UGVs are velocity-controlled, and the optimized contact
forces are used only for feasibility evaluation rather than direct motor control.
Motion capture provides the current robot and object poses, compared with the
successor state predicted by $\mathsf{EvalSim}$: a push triggers replanning only
when the position error exceeds $0.3$\,m or the orientation error exceeds
$\pi/4$, a loose threshold that tolerates slight over-push or off-axis push.}
...
first rotates the yellow L-shaped object to open a gap near an
immovable obstacle and then separates the green and orange objects until a
$W$--clear path is established at $t=52\,$s.
...
Execution takes $77.1$\,s on average, with $15.9$\,s of deviations from
direction-dependent force limits, drifting and Mecanum-wheel slippage, all
compensated by online replanning.
