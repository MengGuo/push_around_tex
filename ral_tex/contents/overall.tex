...
the robots' actuation limits, and the controller tracks the planned contact
poses and pushing velocities with feedback. Planning therefore runs in
simulation through $\mathsf{EvalSim}$, and every executed push returns an
observed state that re-enters the planner, so that physical execution
continuously feeds back into the search.} During transitions,
...
$n_{\texttt{w}}$ parallel workers, so that scalability is mainly determined by
validated pushing candidates, while quick-pass filtering and parallel simulation
reduce the practical expansion cost.} \change{Robot-team size mainly affects mode
generation: the greedy stage evaluates at most $N|\mathcal{A}|$ single-contact
substitutions instead of the exhaustive $|\mathcal{A}|^N$ assignment, and the
bounded-backtracking stage examines at most $B$ additional partial assignments
for a fixed budget~$B$, so the per-task cost is $(N|\mathcal{A}|+B)$ LP
feasibility tests, each over $2N$ force variables.}
%% %----------------------------------------------
