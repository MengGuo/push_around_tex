...
filtering, as illustrated in Fig.~\ref{fig:mode_generation}.
Reachability is checked before mode optimization: a contact candidate is retained only if its assigned robots can reach collision-free pre-contact poses. Starting from
...
The mode is cached as a warm start for similar future queries.
...
The optimized force vector~$\mathbf{u}_m^k$ certifies wrench feasibility; hardware executes pose/velocity commands that maintain the contact geometry.
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/PIHS.pdf}
  \vspace{-2mm}
  \caption{
