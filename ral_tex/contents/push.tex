...
over the admissible contact forces~${U}_m(\mathbf{c})$.
...
replacement. At the $\ell$-th greedy step, let~$\mathbf{c}^{\ell}$ be the
current tuple, and let~$\mathbf{c}_{r,c}^{\ell}$ be obtained by replacing its
$r$-th contact with~$c\in \mathbb{R}^2$. The best replacement is derived by:
\begin{equation}
c^\star
=
...
\end{equation}
where the greedy refinement for~$c^\star$ sweeps over contact slots
until~$\mathrm{J}_{\texttt{MF}}^m$ no longer decreases.
Among initializations, the
best local tuple is selected as~$\mathbf{c}_m^{k,\star}$, thus
$\mathbf{u}_m^k$ is obtained by the inner minimization in
\eqref{eq:mode-feasibility} for~$\mathbf{v}_m^k$.
This yields $\boldsymbol{\xi}_m^k=(\mathbf{c}_m^{k,\star},\mathbf{u}_m^k)$.
...
and the swept region of~$\Omega_m$ is collision-free.
%------------------------------
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/PIHS.pdf}
  \vspace{-2mm}
  \caption{
